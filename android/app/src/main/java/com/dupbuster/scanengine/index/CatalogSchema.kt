 ...
 * [CURRENT_VERSION] is 2 (video metadata + `match_kind` columns); v1→v2 via [CatalogMigrator].
 */
object CatalogSchema {
  const val LEGACY_VERSION: Int = CatalogMigrator.LEGACY_VERSION
  ...
  /** Pre–schema-v2 DDL for migration tests (M1-15). */
  val LEGACY_V1_CREATE_TABLES: Array<String> =
      arrayOf(
          """
          CREATE TABLE IF NOT EXISTS scan_root (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  val LEGACY_V1_CREATE_INDEXES: Array<String> =
      arrayOf(
          "CREATE INDEX IF NOT EXISTS idx_file_entry_size ON file_entry(size)",
          "CREATE INDEX IF NOT EXISTS idx_file_entry_inode_device ON file_entry(inode, device_id)",
          "CREATE INDEX IF NOT EXISTS idx_file_entry_generation ON file_entry(root_id, last_seen_generation)",
  ...
  fun applyLegacyV1Schema(db: android.database.sqlite.SQLiteDatabase, seedMeta: Boolean = true) {
    db.execSQL("PRAGMA foreign_keys=ON")
    for (statement in LEGACY_V1_CREATE_TABLES) {
      db.execSQL(statement)
    }
    for (statement in LEGACY_V1_CREATE_INDEXES) {
      db.execSQL(statement)
    }
    if (seedMeta) {
      db.execSQL(
          "INSERT OR REPLACE INTO meta (key, value) VALUES (?, ?)",
          arrayOf(META_SCHEMA_VERSION, LEGACY_VERSION.toString()),
      )
      db.execSQL(
          "INSERT OR REPLACE INTO meta (key, value) VALUES (?, ?)",
          arrayOf(META_FULL_RESCAN_REQUIRED, "0"),
      )
    }
  }
}
